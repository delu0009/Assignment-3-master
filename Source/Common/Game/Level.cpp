...
void Level::load(const char* levelName)
{
	...
	if(levelName != NULL)
	{
		...
		{
			int tiles = tileTypes[i].get("type", NULL).asInt();
		...
		//Json::Reader reader;
		//
		//Json::Value TileTypes(Json::arrayValue);
		//Json::Value TileType;

		//bool success = reader.parse(levelName,root,false);
		//if(success == true)
		//{
		//	//for (int i = 0; i < get.size(); i++)
		//	//{
		//	//  std::string number = phoneNumbers[i].get(“number”).asString();
		//	//}
		//		for(int i = 0; i < getNumberOfTiles(); i++)
		//		{
...
void Level::save(const char* levelName)
{
	...
	//bool success = reader.parse("somefile.json",root,false);
	root["TileType"] = TileTypes; 
	std::ofstream out;
	out.open(levelName);
	out << root;
	out.close();
...
void Level::setTileTypeAtIndex(TileType tileType, int index)
{
	...
	if(index >= 0 && index < getNumberOfTiles())
	{
		...
		case TileTypePyramid:
			{
				m_Tiles[index] = new PyramidTile();
			}
			break;
			
		case TileTypeMountain:
			{
				m_Tiles[index] = new MountainTile();
			}
			break;
